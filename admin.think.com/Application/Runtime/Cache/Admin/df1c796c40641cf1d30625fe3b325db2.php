...
<div id="main-div">
    <div id="menu-list">
        <ul id="menu-ul">
            <?php if(is_array($menus)): foreach($menus as $key=>$Vla): if(($Vla["level"]) == "1"): ?><li class="explode" key="02_cat_and_goods" name="menu">
                <?php echo ($Vla["name"]); ?>

                <ul>
                    <?php if(is_array($menus)): foreach($menus as $key=>$Vlaue): if(($Vlaue["parent_id"]) == $Vla["id"]): ?><li class="menu-item"><a href="<?php echo U('Goods/goods');?>" target="main-frame"><?php echo ($Vlaue["name"]); ?></a></li><?php endif; endforeach; endif; ?>
                </ul>
